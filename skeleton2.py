...
def range_callback(data):
    ...
    current_depth = data.range

# Set up ROS publisher for usc_asv_msgs/Mission type.
pub = rospy.Publisher('waypoint_cmd', Mission, queue_size=1) #, latch=True)

# Subscribe to sensor data
rospy.Subscriber('sel_gps_fix', NavSatFix, gps_callback)
rospy.Subscriber('depth', Range, range_callback)

# Sets the current depth do a random one
def random_depth(event):
    global current_depth
    current_depth = random.uniform(0.5,10.0)

# Comment this out when using real robot
rospy.Timer(rospy.Duration(1), random_depth)
...
m = pts_to_mission(pts, 'mymission')

# Publish mission
pub.publish(m)

print current_lat, current_lon, current_depth
